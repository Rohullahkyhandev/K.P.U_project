        ...
        <div class="flex items-center justify-between w-full">
            <div>
                <h1 class="text--header">
                    فورم تهیه نمودن راپور اعضای کمیته ها
                </h1>
            </div>
        </div>
            ...
            <div class="wrapper--dev--form">
                <div>
                    ...
                    >
                        <div class="label--dev--width">
                            <label for="" class="form--label">
                                انتخاب کمیته<span class="label--prefix"
                                    >*</span
                                ></label
                            >
                        </div>
...
const ExportUrl = `http://localhost/ViceChancellor/public/postgraduated/report/commit_member/`;

// search student accroding ot the admission year
const commitStore = useCommitMemberStore();
const dateStore = useDateStore();
const years = computed(() => dateStore.years);
const selectedYear = computed(() => dateStore.selectedYear);

onMounted(() => {
    commitStore.getAllCommit();
});
...
const commits = computed(() =>
    commitStore.allCommit.map((commit) => ({
        key: commit.id,
        text: commit.name,
    }))
);
