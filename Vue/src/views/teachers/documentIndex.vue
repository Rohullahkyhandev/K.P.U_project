<template>
    <div class="mt-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <router-link
                    :to="{ name: 'app.teacher.document.create' }"
                    class="header--button"
                >
                    <svg
            ...
            <div>
                <h1 class="text--header">اسناد تحصلی</h1>
            </div>
        </div>

        <!-- display message area -->
        <div class="msg--success" v-if="teacherStore.msg_dsuccess">
            <div class="flex items-center justify-between px-10">
                ...
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        @click="teacherStore.msg_dsuccess = ''"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke-width="1.5"
                        stroke="currentColor"
                ...
                <div>
                    <span>{{ teacherStore.msg_dsuccess }}</span>
                </div>
            </div>
        </div>

        <div class="msg--warning" v-if="teacherStore.msg_dwrang">
            <div class="flex items-center justify-between px-10">
                <div
                ...
                <div>
                    <span>{{ teacherStore.msg_dwrang }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="table--wrapper--dev">
            <div class="flex justify-between border-b-2 pb-3">
                <div>
                    <input
                        ...
                        placeholder="جستجوی بر اساس نوع سند"
                    />
                </div>
                <div class="flex items-center">
                    ...
                    &nbsp;
                    <span class="ml-3">پیداشد {{ documents.total }} کابر</span>
                </div>
            </div>
            <table class="table-auto w-full">
                <thead>
                    <tr>
                        ...
                        <TableHeaderCell
                            ...
                            :sortDirection="sortDirection"
                            :sortField="sortField"
                            @click="sortdocuments('description')"
                        >
                            توضیحات
                        </TableHeaderCell>
                        ...
                        <TableHeaderCell
                            ...
                            :sortDirection="sortDirection"
                            :sortField="sortField"
                            @click="sortdocuments('attachment_path')"
                        >
                            لینک دانلود
                        </TableHeaderCell>
                ...
                <tbody v-if="documents.loading || !documents.data?.length">
                    <tr>
                        <td colspan="6">
                ...
                <tbody v-else>
                    <tr v-for="(document, index) of documents.data">
            ...
            >
                <div v-if="documents.data">
                    نمایش از {{ documents.from }} تا {{ documents.to }}
                </div>
                <nav
                    v-if="documents.total > documents.limit"
                    class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
                    aria-label="Pagination"
                >
                    <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
                    <a
                        v-for="(link, i) of documents.links"
                        :key="i"
                        :disabled="!link.url"
                        href="#"
...
const search = ref("");
const documents = computed(() => teacherStore.teacher_document);
...
function deleteDocument(id) {
    ...
    teacherStore.deleteDocument(id);
    getDocument();
}
