<template>
    <div class="form--padding--top">
        <div class="flex items-center justify-between mb-4">
            ...
            <!-- </div> -->
            <div class="text--header">لیست تحقیقات محصلین</div>
        </div>
        ...
        <div class="table--wrapper--dev">
            ...
            <div
                ...
                v-if="studentResearchStore.msg_wrang"
            >
                <div class="flex items-center justify-between px-10">
                    <div
            ...
            <div class="flex justify-between border-b-2 pb-3">
                <div>
                    <input
                        v-model="search"
                        @change="getStudentResearch(null)"
                ...
                <div class="flex items-center">
                    ...
                    &nbsp;
                    <span class="ml-3"
                        >پیداشد {{ studentResearchs.total }} دیتا</span
                    >
                </div>
            </div>
            ...
            <table class="table-auto w-full border">
                ...
                <tbody
                    v-if="
                        studentResearchs.loading ||
                        !studentResearchs.data.length
                    "
                >
                    <tr>
                        <td colspan="10">
                            <Spinner v-if="studentResearchs.loading" />
                ...
                <tbody class="border" v-else>
                    <tr
                        v-for="(
                            studentResearch, index
                        ) of studentResearchs.data"
                        :key="index"
                    >
                        <td class="border-b p-3">{{ index + 1 }}</td>

                        <td class="border p-">
                            {{ studentResearch.name }}
                            {{ studentResearch.lname }}
                        </td>

                        <td class="border p-">
                            {{ studentResearch.teacher_name }}
                            {{ studentResearch.teacher_lname }}
                        </td>

                        <td class="border p-">
                            {{ studentResearch.program }}
                        </td>
                        ...
                        <td class="border p-3">
                            {{ studentResearch.date }}
                        </td>

                        <td class="border p-3">
                            {{ studentResearch.description }}
                        </td>

                        <td class="border p-3">
                            <a :href="studentResearch.attachment_path"
                                >دانلود اسناد</a
                            >
                        </td>

                        <td class="border p-3">
                            {{ studentResearch.uname }}
                        </td>
                                    ...
                                    >
                                        <div class="px-1 py-1">
                                            <MenuItem v-slot="{ active }">
                                                <a
                                                    :href="
                                                        studentResearch.attachment_path
                                                    "
                                                    :class="[
                                                        active
            ...
            <div
                v-if="!getStudentResearch.loading"
                class="flex justify-between items-center mt-5"
                dir="ltr"
            >
                <div v-if="getStudentResearch.data">
                    نمایش از {{ getStudentResearch.from }} تا
                    {{ getStudentResearch.to }}
                </div>
                <nav
                    v-if="getStudentResearch.total > getStudentResearch.limit"
                    class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
                    aria-label="Pagination"
                >
                    <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
                    <a
                        v-for="(link, i) of getStudentResearch.links"
                        :key="i"
                        :disabled="!link.url"
                        href="#"
                        ...
                        :class="[
                            ...
                            i === 0 ? 'rounded-l-md' : '',
                            i === studentResearchs.links.length - 1
                                ? 'rounded-r-md'
                                : '',
...
import usestudentResearchStore from "../../../stores/postgraduatedPrograms/programStore";
import useStudentResearchStore from "../../../stores/postgraduatedPrograms/studentResearch";
import useProgramStore from "../../../stores/postgraduatedPrograms/programStore";
import useDateStore from "../../../stores/dateStore";
const studentResearchStore = useStudentResearchStore();
...
const search = ref("");
const studentResearchs = computed(() => studentResearchStore.studentResearchs);
...
const title = ref("فورم ویرایش اطلاعات محصیلن تازه فارغ");
...
function getForPage(ev, link) {
    ...
    getStudentResearch(link.url);
}

function getStudentResearch(url = null) {
...
function deleteStudentResearch(id) {
    ...
    studentResearchStore.deleteStudentResearch(id);
