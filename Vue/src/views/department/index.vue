<template>
    <div class="form--padding--top">
        <div class="flex items-center justify-between mb-8">
            ...
            <div>
                <h1 class="text--header">لیست کلی دیپارتمنت ها</h1>
            </div>
        </div>
        <div class="table--wrapper--dev w-full">
            ...
            <div class="msg--warning" v-if="departmentStore.msg_wrang">
            ...
            <div class="flex justify-between border-b-2 pb-3 rounded-lg">
                <div>
            ...
            <table class="table-auto w-full border">
                <thead>
                    <tr>
                        ...
                        <TableHeaderCell field="manger_name" :sortDirection="sortDirection" :sortField="sortField"
                            @click="sortUsers('manger_name')">
                            آمر دیپارتمنت
                        </TableHeaderCell>
                ...
                <tbody v-if="
                    faculty_departments.loading ||
                    !faculty_departments.data?.length
">
                    <tr>
                        <td colspan="7">
                ...
                <tbody v-else>
                    <tr v-for="(
                            faculty_department, index
                        ) of faculty_departments.data" :key="index">
                        ...
                        <td class="border p-2">
                            {{ faculty_department.manger_name }}
                            {{ faculty_department.manager_lname }}
                        </td>
                        <td class="border p-2">
                            <img :src="faculty_department.photo_path" class="object-fill w-20 h-18"
                                :alt="faculty_department.manger_name" />
                        </td>
                        ...
                        <td class="border p-2 text-center">
                            <Menu as="div" class="relative inline-block text-left">
                                <div>
                                    <MenuButton
                                        class="inline-flex items-center rounded-full  h-10 bg-black bg-opacity-0 text-sm font-medium text-blue-800 hover:bg-opacity-5 focus:bg-opacity-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
            ...
            <div v-if="!faculty_departments.loading" class="flex justify-between items-center mt-5" dir="ltr">
                <div v-if="faculty_departments.data">
                    نمایش از {{ faculty_departments.from }} تا
                    {{ faculty_departments.to }}
                </div>
                <nav v-if="faculty_departments.total > faculty_departments.limit"
                    ...
                    <a v-for="(link, i) of faculty_departments.links" :key="i" :disabled="!link.url" href="#"
                        ...
                        :class="[
                            ...
                            i === 0 ? 'rounded-l-md' : '',
                            i === faculty_departments.links.length - 1
                                ? 'rounded-r-md'
                                : '',
...
function deleteDepartment(id) {
    if (!confirm(`شما  وقعاً می خواهید این دیتا را حذف نماید?`)) {
        return;
    }
