        ...
        <div class="table--wrapper--dev">
            ...
            <div class="flex justify-between border-b-2 pb-3">
                ...
                <div class="flex items-center">
                    ...
                    &nbsp;
                    <span class="mr-4"
                        >پیداشد {{ teacherInCommits.total }} دیتا</span
                    >
                </div>
            </div>
            ...
            <table class="table-auto w-full border">
                <thead>
                    <tr>
                        ...
                        <TableHeaderCell
                            ...
                            :sortDirection="sortDirection"
                            :sortField="sortField"
                            @click="storeTeacherInCommit('teacher_id')"
                        >
                            نام
                        </TableHeaderCell>
                        ...
                        <TableHeaderCell
                            ...
                            :sortDirection="sortDirection"
                            :sortField="sortField"
                            @click="storeTeacherInCommit('commit_id')"
                        >
                            تخلص
                        </TableHeaderCell>
                        ...
                        <TableHeaderCell
                            ...
                            :sortDirection="sortDirection"
                            :sortField="sortField"
                            @click="storeTeacherInCommit('acs')"
                        >
                            عملیات
                        </TableHeaderCell>
                    </tr>
                ...
                <tbody
                    v-if="
                        teacherInCommits.loading ||
                        !teacherInCommits.data?.length
                    "
                >
                    <tr>
                        <td colspan="10">
                            <Spinner v-if="teacherInCommits.loading" />
                ...
                <tbody class="border" v-else>
                    ...
                    >
                        ...
                        <td class="border p-">
                            {{ teacherInCommit.name }}
                        </td>

                        <td class="border p-3">
                            {{ teacherInCommit.lname }}
                        </td>

                        <td class="border p-3">
                            {{ teacherInCommit.email }}
                        </td>
                        <td class="border p-3">
                        ...
                        <td class="border p-3">
                            <a
                                ...
                                :href="
                                    teacherInCommit.attachment_path
                                        ? teacherInCommit.attachment_path
                                        : '#'
                                "
                            >
            ...
            <div
                v-if="!teacherInCommits.loading"
                class="flex justify-between items-center mt-5"
                dir="ltr"
            >
                <div v-if="teacherInCommits.data">
                    نمایش از {{ teacherInCommits.from }} تا
                    {{ teacherInCommits.to }}
                </div>
                <nav
                    v-if="teacherInCommits.total > teacherInCommits.limit"
                    class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
                    aria-label="Pagination"
                >
                    <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
                    <a
                        v-for="(link, i) of teacherInCommits.links"
                        :key="i"
                        :disabled="!link.url"
                        href="#"
                        ...
                        :class="[
                            ...
                            i === 0 ? 'rounded-l-md' : '',
                            i === teacherInCommits.links.length - 1
                                ? 'rounded-r-md'
                                : '',
...
const search = ref("");
const teacherInCommits = computed(
    () => teacherInCommitStore.teacher_in_commits
);
...
function getForPage(ev, link) {
    ...
    getTeacherInCommit(link.url);
}

function getTeacherInCommit(url = null) {
    teacherInCommitStore.getTeacherInCommit({
        url,
        search: search.value,
...
function storeTeacherInCommit(field) {
...
function deleteTeacherInCommit(id) {
    if (!confirm(`آیا شما می خواهید این دیتا را خذف نماید?`)) {
        return;
    }
    teacherInCommitStore.deleteTeacherInCommit(id);
    if (teacherInCommitStore.msg_success != null) {
    }
    getTeacherInCommit();
}
