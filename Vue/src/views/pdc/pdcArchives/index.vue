<template>
    <div class="px-5 mt-10">
        <div class="flex items-center justify-between mb-8">
            <div>
                <router-link :to="{ name: 'app.pdc.archive.create' }"
                    class="bg-blue-600 py-3 px-3 flex items-center justify-center gap-3 rounded shadow text-white focus:ring focus:ring-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5 text-white">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    ثبت اطلاعات در آرشیف
                </router-link>
            </div>
            <div>
                <h1 class=" font-bold text-xl ">لیست اطلاعات در آرشیف</h1>
            </div>
        </div>

        <div class="bg-white p-6 rounded shadow animate-fade-in-down overflow-auto">
            <!-- display message area -->
            <div class="bg-green-700 text-white rounded py-4 text-center mb-3" v-if="archiveStore.msg_success">
                <div class="flex items-center justify-between px-10">
                    <div
                        class="hover:bg-green-400 text-white rounded-full h-8 w-8 cursor-pointer flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" @click="archiveStore.msg_success = ''" fill="none"
            ...
            <div class="bg-red-500 text-white py-4 rounded text-center mb-3" v-if="archiveStore.msg_wrang">
                <div class="flex items-center justify-between px-10">
                    <div
                        class="hover:bg-red-300 text-white rounded-full h-8 w-8 cursor-pointer flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" @click="archiveStore.msg_wrang = ''" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                        </svg>
                    </div>
            ...
            <div class="flex justify-between border-b-2 pb-3">
                ...
                <div>
                    <input v-model="search" @change="getArchive(null)"
                ...
                <div class="flex items-center">
                    ...
                    &nbsp;
                    <select dir="ltr" @change="getArchive(null)" v-model="perPage"
                    ...
                    &nbsp;
                    <span class="ml-3">پیداشد {{ archiveStore.total }} دیتا</span>
                </div>
            </div>
            <table class="table-auto w-full">
                ...
                <tbody v-if="archives.loading || !archives.data?.length">
                    <tr>
                        <td colspan="7">
                            <Spinner v-if="archives.loading" />
                ...
                <tbody v-else>
                    <tr v-for="(archive, index) of archives.data">
                                ...
                                <transition enter-active-class="transition duration-100 ease-out"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="px-1 py-1">
                                            ...
                                            <router-link :to="{ name: 'app.pdc.archive.edit', params: { id: archive.id } }"
                                                :class="[
                                        ...
                                        <div class="px-1 py-1">
                                            <MenuItem v-slot="{ active }">
                                            <button @click="deleteArchive(archive.id)" :class="[
                                                active ? 'bg-blue-800 text-white' : 'text-gray-900',
                                                'group flex w-full items-center rounded-md gap-3 px-2 py-2 text-sm',
                                            ]">
            ...
            <div v-if="!archives.loading" class="flex justify-between items-center mt-5" dir="ltr">
                <div v-if="archives.data">
                    نمایش از {{ archives.from }} تا {{ archives.to }}
                </div>
                <nav v-if="archives.total > archives.limit"
                    class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px"
                    aria-label="Pagination">
                    <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
                    <a v-for="(link, i) of archives.links" :key="i" :disabled="!link.url" href="#"
                        ...
                        :class="[
                            ...
                            i === 0 ? 'rounded-l-md' : '',
                            i === archives.links.length - 1 ? 'rounded-r-md' : '',
