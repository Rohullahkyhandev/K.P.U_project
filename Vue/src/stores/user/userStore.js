...
export const useUserStore = defineStore('user', () => {
    ...
    function getUsers({ url = null, search = '', per_page, sortField, sortDirection, } = {}) {
    ...
    function createUser(data) {
        ...
        data = form;
        axiosClient.post('/user/create', data)
            .then(res => {
    ...
    function getUserPermission(id) {
        axiosClient.get(`/permission/list/${id}`)
            .then((res) => {
    ...
    function deleteUser(id) {
        axiosClient.get(`/user/delete/${id}`)
            .then((res) => {
                console.log(res);
                if (res.status === 200) {
    ...
    function createPermission(data) {
        axiosClient.post('/permission/store', data)
            .then((res) => {
                msg_success.value = res.data.message;
            }).catch((err) => {
    ...
    function getCurrentPermission() {
        axiosClient.get('/user/current/permission')
            .then(({ data }) => {
                console.log(data);
                data.forEach(item => {
                    console.log(item.id);
    ...
    function deletePermission(user_id) {
        axiosClient.get(`/permission/delete/${user_id}`)
            .then((res) => {
                console.log(res);
                if (res.status === 200) {
