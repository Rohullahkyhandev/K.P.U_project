<template>
    <div>
        ...
        <div class="mt-1 flex shadow rounded-md">
            ...
            <template v-if="type === 'file' && size === 'multi'">
                <input :type="type" :id="id" multiple :required="required" @input="emit('change', $event.target.files)"
            ...
            <template v-else-if="type === 'select'">
                <select dir="ltr" :id="id" :value="props.modelValue" @change="onChange($event.target.value)"
                    ...
                    <option value="" disabled selected class="flex items-center justify-between float-end">
                        انتخاب
                        {{ label }}
                    </option>
            ...
            <template v-else-if="type === 'select2'">
                <select dir="ltr" :id="id" multiple :value="props.modelValue" @change="onChange($event.target.value)"
                    :required="required" :name="name" :class="inputClasses">
                    <option value="" disabled selected class="flex items-center justify-between float-end">
                        انتخاب
                        {{ label }}
